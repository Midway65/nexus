...
**Mar 29**: Chat Action Buttons ✅ (local-fixes branch, commit 21c260f3 + cleanup)
- New `MessageActionBar` adds insert-at-cursor, append-to-file, create-new-file buttons into the existing `message-actions-external` pill on every assistant bubble alongside the copy button
- Pill starts at `opacity: 0.35` (was `0`) for assistant messages — always discoverable, solidifies on hover (pure CSS)
- New `EditorInsertService`: `insertAtCursor` uses `iterateAllLeaves` fallback so it works when chat side panel has focus (not just `activeEditor`); `appendToActiveFile` and `createNewFile` use `vault.process` and `vault.create`
- New `CreateFileModal`: filename + folder + backlink toggle; folder pre-filled from `defaultNewFileLocation` setting; opens created file via `getLeaf(false).openFile()`; uses typed `PluginWithFileSettings` interface (no `any` cast)
- `PluginTypes.MCPSettings`: added `defaultNewFileLocation?: string`
- `DefaultsTab`: "Chat actions" section with default folder text field
- `postbuild.ps1` created — `npm run deploy` now builds and copies to vault in one step
- **Key pattern**: `Modal` TypeScript definitions do not expose `registerDomEvent` even though `Modal extends Component`; use `addEventListener` in Modal classes (matches all existing modals in the project)
...
| registerDomEvent | ~27 raw addEventListener (modals + view components) | ~27 to fix (modals: `Modal` TS types don't expose `registerDomEvent` — `addEventListener` is accepted pattern for all Modal subclasses) |
...
| Type safety (`as any`) | Regressed from 0 → 16; GithubCopilotAdapter adds 7 more; CreateFileModal fixed | ~22 to fix |
