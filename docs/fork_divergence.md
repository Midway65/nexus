...
| `src/database/adapters/HybridStorageAdapter.ts` | 30s `fullRebuild()` timeout added in `performInitialization()` (PR #158 fix). All 3 vault-root fork patches removed — upstream behavior restored. | Take upstream base; re-apply the `Promise.race()` timeout block around `syncCoordinator.fullRebuild()`. See commit `2acd2570` for exact diff. |
