...
**Last audited against:** upstream/main HEAD (`aafcfcce`) — v5.16.4 (Bases support #330, notes-index schema ownership + **first upstream migration since the fork diverged**, startup-hydration fix, search-ranking overhaul, CLI multiline transports, Defuddle web capture, Gemini 3.7 Flash + DeepSeek V4 Pro)  
...
|------|-------------|----------------------|
| `src/database/schema/SchemaMigrator.ts` | Convention comment + fork migrations v17–v19 (v12–v16 removed 2026-04-08) + v20 (renumbered from upstream v12 `shard_cursors`, merged 2026-05-07) + v21 (renumbered from upstream v13 `skills`, merged 2026-05-22) + **v22 (renumbered from upstream v14 `notes`/`note_properties` query index, merged 2026-08-14)**. `CURRENT_SCHEMA_VERSION = 22`. | Restore convention comment block + migrations v17–v22. When upstream ships their next migration (v15 in upstream numbering), renumber it to 23 and set `CURRENT_SCHEMA_VERSION = 23`. Upstream is at v14; they need 8 more before their counter exceeds the fork MAX and renumbering can stop. **Precedent: the v14→v22 renumber (v5.16.4) is the worked example — conflict is only on the `CURRENT_SCHEMA_VERSION` line; upstream's new migration block auto-merges to the END of the array (out of order), so renumber it in place, prefix the description `[upstream vN]`, and add a renumber comment.** |
...
| `tests/unit/SchemaMigrator.test.ts` | Numeric assertions track v20 + v21 + **v22** renumbers (upstream tests them at v12 + v13 + v14). "Starting at v19" test asserts v20+v21+v22 run (applied=3, toVersion=22); "starting at v20" asserts v21+v22 (applied=2); no-op test seeds MAX=22. Three describe blocks mirroring upstream's, all renumbered. **11 tests.** (last touched in v5.16.4 merge, 2026-08-14) | Will need re-adjustment whenever upstream lands another migration that the fork renumbers: bump the `CURRENT_SCHEMA_VERSION` assertion, extend the applied-set/`toVersion` expectations in the earlier describes, and renumber upstream's new describe block. |
