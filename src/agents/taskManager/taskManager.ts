...
export class TaskManagerAgent extends BaseAgent {
  ...
  constructor(app: App, plugin: NexusPlugin, taskService: TaskService) {
    ...
    this.registerLazyTool({
      ...
      description: 'Update task fields (title, description, status, priority, dueDate, assignee, tags), manage DAG dependencies (addDependencies/removeDependencies). Dependency additions are validated for cycles. Requires a taskId (from createTask or listTasks).',
      version: '1.0.0',
